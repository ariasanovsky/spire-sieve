...
impl Display for Map {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        .map(|row| {
            row
            .iter()
            .map(|(in_neighborhood, _, _)| {
                in_neighborhood
            })
            .enumerate()
            .map(|(i, in_neighborhood)| {
                EnumeratedInNeighborhood(in_neighborhood, i)
            })
        })
        .enumerate()
        .rev()
...
impl Display for EnumeratedInNeighborhood<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match &neighbors[..] {
            [] => write!(f, "   ")?,
            [one] => write!(f, "{} ", match position.cmp(one) {
                std::cmp::Ordering::Less =>   r"  \",
                std::cmp::Ordering::Equal =>   " | ",
                std::cmp::Ordering::Greater => "/  ",
            })?,
            [_, two] => write!(f, "{} ", match position.cmp(two) {
                std::cmp::Ordering::Less =>   r" |\",
