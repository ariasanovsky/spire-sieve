...
use crate::{seed, card::{Card, card_pool_range, Rarity}, character::Character};
...
impl<const CHARACTER: usize, const UNLOCKS: usize> PandorasBoxFilter<CHARACTER, UNLOCKS> {
    pub fn pandoras_box(&self, seed: &seed::Seed) -> [Card; 9] {
        ...
        array::from_fn(|_| {
            CARDS[rng.next_capped_u64(N as u64) as usize]
        })
    }
...
fn test_pandoras_box() {
    ...
    const FILTER: PandorasBoxFilter<{Character::Ironclad as usize}, {Unlocks::full().encode()}> = PandorasBoxFilter;
